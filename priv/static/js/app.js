...
;require.register("web/static/elm/lib/elm-phoenix/src/Native/Phoenix", function(exports, require, module) {
'use strict';
...
Elm.Native.Phoenix.make = function (localRuntime) {
	...
	function newSocket(options) {
		...
		if (options.heartbeatIntervalMs.ctor == 'Just') opts.heartbeatIntervalMs = options.heartbeatIntervalMs._0;
			...
			switch (options.logger._0.ctor) {
				case 'Console':
					opts.logger = function (kind, message, data) {
						if (data) console.log(kind + ": " + message, data);else console.log(kind + ": " + message);
					};
					break;
				case 'Mailbox':
		...
		if (options.longpollerTimeout.ctor == 'Just') opts.longpollerTimeout = options.longpollerTimeout._0;

		if (options.params.ctor == 'Just') opts.params = options.params._0;
		...
		if (options.onStateChange.ctor == 'Just') {
			...
			var reportStateChange = function reportStateChange(state) {
	...
	function newChannel(options) {
		var channel = options.params.ctor == 'Just' ? options.socket.channel(options.topic, options.params._0) : options.socket.channel(options.topic);
	...
	return localRuntime.Native.Phoenix.values = {
		...
		tryConnect: function tryConnect(options) {
			return Task.asyncFunction(function (callback) {
				var socket = newSocket(options);
				var trying = true;
				socket.onOpen(function () {
		...
		tryJoin: F2(function (timeout, options) {
			return Task.asyncFunction(function (callback) {
				...
				var joinPush = channel.join();
				var trying = true;
				var reportStateChange = function reportStateChange() {};
				if (options.onStateChange.ctor == 'Just') {
					var statusChangeCallback = options.onStateChange._0;
		...
		tryPush: F4(function (event, payload, timeout, channel) {
			return Task.asyncFunction(function (callback) {
				channel.push(event, payload).receive("ok", function (msg) {
					Task.succeed(msg);
				}).receive("error", function (reason) {
					Task.fail({ ctor: 'ErrorReply', _0: reason });
				}).after(timeout, function () {
					Task.fail({ ctor: 'Timeout' });
				});
			});
		}),
		...
		socketToString: function socketToString(socket) {
			return "Phoenix.Socket< endpoint=" + socket.endPointURL() + " >";
		},
		channelToString: function channelToString(channel) {
			return "Phoenix.Channel< topic=" + channel.topic + " >";
		}
	};
};
});
...
window.Phoenix.Socket = window.Phoenix.Socket || _depsPhoenixWebStaticJsPhoenix.Socket;
window.Phoenix.LongPoll = window.Phoenix.LongPoll || _depsPhoenixWebStaticJsPhoenix.LongPoll;

var chatApp = Elm.embed(Elm.PhoenixChat, document.getElementById("elm-main"), {});
});
